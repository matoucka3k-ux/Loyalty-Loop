...
export default function Onboarding() {
  ...
  const btnPrimary = {
    ...
    border: 'none',
    cursor: 'pointer',
    boxShadow: '0 8px 24px -8px rgba(30,64,175,0.5)',
  }
          ...
          {[1,2,3].map((s,i) => (
            <div key={s} style={{display:'flex',alignItems:'center',gap:8}}>
              <div style={{
                ...
                background:step>=s?'linear-gradient(135deg,#1e40af,#3b82f6)':'#e7e5e4',
              ...
              {i < 2 && (
                <div style={{width:48,height:2,background:step>s?'#3b82f6':'#e7e5e4',transition:'all 0.3s'}} />
              )}
            </div>
          ))}
        </div>
        ...
        {step === 1 && (
          <div style={{background:'white',borderRadius:24,padding:40,border:'1px solid #dbeafe',boxShadow:'0 20px 60px -20px rgba(30,64,175,0.15)',textAlign:'center'}}>
            ...
            <p style={{color:'#78716c',fontSize:16,marginBottom:8}}>
              <strong style={{color:'#1e40af'}}>{merchant?.business_name}</strong> est sur FideliPain.
            </p>
            <p style={{color:'#a8a29e',fontSize:14,marginBottom:32}}>
              Configurons votre programme en 2 minutes.
            </p>
        ...
        {step === 2 && (
          <div style={{background:'white',borderRadius:24,padding:40,border:'1px solid #dbeafe',boxShadow:'0 20px 60px -20px rgba(30,64,175,0.15)'}}>
            <div style={{textAlign:'center',marginBottom:32}}>
              <h2 style={{fontSize:22,fontWeight:800,color:'#1e3a5f',marginBottom:8}}>
                Choisissez votre systeme de points
              </h2>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:28}}>
              <button
                onClick={() => setLoyaltyMode('euro')}
                style={{
                  padding:'20px 16px',borderRadius:16,cursor:'pointer',textAlign:'center',transition:'all 0.2s',
                  border:loyaltyMode==='euro'?'2px solid #3b82f6':'2px solid #dbeafe',
                  background:loyaltyMode==='euro'?'#eff6ff':'#f8faff',
                }}>
                <div style={{
                  width:44,height:44,borderRadius:12,margin:'0 auto 10px',
                  display:'flex',alignItems:'center',justifyContent:'center',
                  background:loyaltyMode==='euro'?'#dbeafe':'#e7e5e4',
                }}>
                  <Euro style={{width:22,height:22,color:loyaltyMode==='euro'?'#1e40af':'#a8a29e'}} />
                </div>
                <p style={{fontWeight:700,fontSize:14,marginBottom:4,color:loyaltyMode==='euro'?'#1e40af':'#1e3a5f'}}>Par euro</p>
                <p style={{fontSize:12,color:'#78716c'}}>Ex: 1 = 2 pts</p>
              </button>

              <button
                onClick={() => setLoyaltyMode('products')}
                style={{
                  padding:'20px 16px',borderRadius:16,cursor:'pointer',textAlign:'center',transition:'all 0.2s',
                  border:loyaltyMode==='products'?'2px solid #3b82f6':'2px solid #dbeafe',
                  background:loyaltyMode==='products'?'#eff6ff':'#f8faff',
                }}>
                <div style={{
                  width:44,height:44,borderRadius:12,margin:'0 auto 10px',
                  display:'flex',alignItems:'center',justifyContent:'center',
                  background:loyaltyMode==='products'?'#dbeafe':'#e7e5e4',
                }}>
                  <ShoppingBag style={{width:22,height:22,color:loyaltyMode==='products'?'#1e40af':'#a8a29e'}} />
                </div>
                <p style={{fontWeight:700,fontSize:14,marginBottom:4,color:loyaltyMode==='products'?'#1e40af':'#1e3a5f'}}>Par produit</p>
            ...
            {loyaltyMode === 'euro' && (
              <div>
                <p style={{fontWeight:600,color:'#1e3a5f',fontSize:14,marginBottom:12}}>Points par euro depense</p>
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:16}}>
                  {[1,2,5,10].map(p => (
                    <button
                      key={p}
                      onClick={() => setPointsPerEuro(p)}
                      style={{
                        padding:'14px',borderRadius:12,fontWeight:700,fontSize:18,cursor:'pointer',
                        border:pointsPerEuro===p?'2px solid #3b82f6':'2px solid #dbeafe',
                        background:pointsPerEuro===p?'#eff6ff':'#f8faff',
                        color:pointsPerEuro===p?'#1e40af':'#1e3a5f',
                      }}>
                      {p}
                    </button>
                  ))}
                ...
                <div style={{background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:12,padding:14,textAlign:'center'}}>
                  <p style={{fontSize:14,color:'#1e3a5f'}}>
                    Pour <strong>10</strong> depenses, client gagne <strong style={{color:'#1e40af'}}>{pointsPerEuro * 10} points</strong>
                  </p>
                </div>
              </div>
            )}
            ...
            {loyaltyMode === 'products' && (
              <div>
                <p style={{fontWeight:600,color:'#1e3a5f',fontSize:14,marginBottom:12}}>Vos produits</p>
                  ...
                  {products.map((p, i) => (
                    <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:'#f8faff',borderRadius:12,border:'1px solid #dbeafe'}}>
                      ...
                      <button
                        ...
                        style={{background:'none',border:'none',cursor:'pointer',color:'#e11d48',padding:4}}>
                ...
                <div style={{display:'flex',gap:8,marginBottom:8}}>
                  ...
                  <input
                    ...
                    onChange={e => setNewProduct(p => ({...p, points: e.target.value}))}
                    placeholder="Pts"
                    className="input-base"
                    style={{flex:1}}
                  />
                ...
                <p style={{fontSize:12,color:'#a8a29e'}}>Ajoutez plus de produits depuis le dashboard.</p>
              </div>
            )}

            <button
              onClick={saveConfig}
              disabled={saving || (loyaltyMode === 'products' && products.length === 0)}
              style={{
                ...btnPrimary,
                marginTop:24,
                fontSize:15,
        ...
        {step === 3 && (
          <div style={{background:'white',borderRadius:24,padding:40,border:'1px solid #dbeafe',boxShadow:'0 20px 60px -20px rgba(30,64,175,0.15)',textAlign:'center'}}>
            ...
            <p style={{color:'#78716c',fontSize:14,marginBottom:32}}>
              Affichez-le en caisse. Vos clients le scannent pour rejoindre votre programme.
            </p>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:32}}>
              {[
                loyaltyMode === 'euro'
                  ? pointsPerEuro + ' point' + (pointsPerEuro > 1 ? 's' : '') + ' par euro configure'
                  : products.length + ' produits configures',
                'QR code pret a afficher',
                'Programme actif immediatement',
              ].map((text, i) => (
