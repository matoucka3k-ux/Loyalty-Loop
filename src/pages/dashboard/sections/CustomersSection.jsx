...
export default function CustomersSection() {
  ...
  const handleAddPoints = async () => {
    ...
    try {
      await supabase.from('customers').update({ points: selectedCustomer.points + Number(pointsToAdd) }).eq('id', selectedCustomer.id)
      await supabase.from('transactions').insert({ customer_id: selectedCustomer.id, merchant_id: merchant.id, points: Number(pointsToAdd), type: 'earn', description: 'Points ajoutés par le commerçant' })
      setCustomers(cs => cs.map(c => c.id === selectedCustomer.id ? {...c, points: c.points + Number(pointsToAdd)} : c))
      success(`${pointsToAdd} points ajoutés !`)
        ...
        {selectedCustomer && (
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            ...
            <div style={{display:'flex',gap:12,paddingTop:4}}>
              <button onClick={() => {setSelectedCustomer(null);setPointsToAdd('')}} className="btn-secondary" style={{flex:1,justifyContent:'center'}}>Annuler</button>
              <button onClick={handleAddPoints} disabled={addingPoints||!pointsToAdd} className="btn-primary" style={{flex:1,justifyContent:'center'}}>
