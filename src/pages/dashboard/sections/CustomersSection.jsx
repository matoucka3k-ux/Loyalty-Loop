...
export default function CustomersSection() {
  ...
  const startScanner = async () => {
    ...
    try {
      ...
      toastError("Impossible d'acceder a la camera")
      setScanning(false)
    }
  }
      ...
      if (data.customerId) {
        ...
        if (!customer) {
          ...
          if (fresh) {
            ...
            toastError("Ce client n'appartient pas a votre boulangerie")
            return
          }
        }
        openModal(customer)
  ...
  const handleAddPoints = async () => {
    ...
    try {
      const desc = loyaltyMode === 'products'
        ...
        : 'Points ajoutes par le commercant'
      ...
      success('+' + pts + ' points attribues a ' + selectedCustomer.profiles?.full_name + ' !')
  ...
  const pointsBg = '#eff6ff'
  const pointsBorder = '1px solid #bfdbfe'
    ...
    <div className="animate-mount">
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24}}>
        ...
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <div style={{display:'flex',alignItems:'center',gap:8,padding:'8px 12px',background:pointsBg,borderRadius:10,border:pointsBorder}}>
            {loyaltyMode === 'euro'
          ...
          <button
            ...
            style={{
              ...
              background: scanning ? '#e11d48' : 'linear-gradient(135deg,#1e40af,#3b82f6)',
              color:'white',fontWeight:600,fontSize:14,borderRadius:10,border:'none',cursor:'pointer'
            }}>
            {scanning
              ? <><X style={{width:16,height:16}} /> Arreter</>
      ...
      {scanning && (
        <div style={{background:'white',borderRadius:16,border:'2px solid #3b82f6',marginBottom:16,overflow:'hidden'}}>
          ...
          <div style={{padding:16,position:'relative'}}>
            ...
            <p style={{textAlign:'center',fontSize:13,color:'#78716c',marginTop:12}}>
              Dirigez la camera vers le QR code du client
            </p>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div style={{background:'white',borderRadius:16,border:'1px solid #dbeafe',padding:48,textAlign:'center'}}>
          ...
          <p style={{color:'#a8a29e',fontSize:14,marginBottom:20}}>
            Vos clients apparaitront ici apres avoir scanne votre QR code
          </p>
          <button
            onClick={startScanner}
            style={{display:'inline-flex',alignItems:'center',gap:8,padding:'10px 20px',background:pointsBg,color:'#1e40af',fontWeight:600,fontSize:14,borderRadius:10,border:pointsBorder,cursor:'pointer'}}>
        ...
        <div style={{background:'white',borderRadius:16,border:'1px solid #dbeafe',overflow:'hidden'}}>
          <div style={{
            ...
            padding:'12px 20px',background:'#f8faff',borderBottom:'1px solid #dbeafe',
            fontSize:12,fontWeight:500,color:'#a8a29e',textTransform:'uppercase',letterSpacing:'0.05em'
          }}>
            <span>Client</span>
          ...
          {filtered.map((customer, i) => (
            <div key={customer.id} style={{
              display:'grid',gridTemplateColumns:'1fr auto auto auto',gap:16,
              alignItems:'center',padding:'16px 20px',borderBottom:'1px solid #f8faff'
            }}>
              <div style={{display:'flex',alignItems:'center',gap:12,minWidth:0}}>
                <div style={{
                  width:36,height:36,borderRadius:'50%',background:pointsBg,
                  display:'flex',alignItems:'center',justifyContent:'center',
                  fontWeight:700,color:'#1e40af',fontSize:14,flexShrink:0
                }}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    ...
                    {i === 0 && customers.length > 1 && (
                      <span style={{
                        ...
                        background:'#fefce8',color:'#854d0e',borderRadius:99,fontSize:11,fontWeight:500,flexShrink:0
                      }}>
                        <TrendingUp style={{width:10,height:10}} /> Top
                      </span>
                    )}
                  </div>
                  <p style={{fontSize:12,color:'#a8a29e',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                    {customer.profiles?.email}
                  </p>
                </div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',background:pointsBg,borderRadius:8}}>
              ...
              <button
                ...
                style={{display:'flex',alignItems:'center',gap:6,padding:'6px 12px',background:pointsBg,border:'none',borderRadius:8,cursor:'pointer',color:'#1e40af',fontSize:13,fontWeight:500}}>
        ...
        {selectedCustomer && (
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{display:'flex',alignItems:'center',gap:12,padding:16,background:'#f8faff',borderRadius:12}}>
              <div style={{
                width:40,height:40,borderRadius:'50%',background:pointsBg,
                display:'flex',alignItems:'center',justifyContent:'center',
                fontWeight:700,color:'#1e40af',fontSize:16
              }}>
            ...
            {loyaltyMode === 'euro' && (
              <>
                <div>
                  <label style={{display:'block',fontSize:14,fontWeight:500,color:'#44403c',marginBottom:6}}>
                    Points a ajouter
                  </label>
                  <div style={{position:'relative'}}>
                    <input
                      type="number" min="1" value={pointsToAdd}
                ...
                <div style={{display:'flex',gap:8}}>
                  {[10, 25, 50, 100].map(p => (
                    <button key={p} onClick={() => setPointsToAdd(p.toString())}
                      style={{
                        flex:1,padding:'8px',borderRadius:10,border:'none',fontSize:13,fontWeight:500,cursor:'pointer',
                        background: String(pointsToAdd) === String(p) ? '#1e40af' : pointsBg,
                        color: String(pointsToAdd) === String(p) ? 'white' : '#1e40af'
                      }}>
                      +{p}
                    </button>
                  ))}
                ...
                {pointsToAdd && (
                  <div style={{background:pointsBg,border:pointsBorder,borderRadius:12,padding:12,fontSize:14,color:'#1e3a5f'}}>
            ...
            {loyaltyMode === 'products' && (
              <>
                <div>
                  <label style={{display:'block',fontSize:14,fontWeight:600,color:'#1e3a5f',marginBottom:10}}>
                    Produits achetes
                  </label>
                    ...
                    {products.map((product, i) => {
                      ...
                      return (
                        <div key={i} style={{
                          ...
                          padding:'12px 14px',background:'#f8faff',borderRadius:12,border:'1px solid #dbeafe'
                        }}>
                          <div style={{display:'flex',alignItems:'center',gap:10}}>
                            <span style={{fontSize:18}}>🥐</span>
                            <div>
                              <p style={{fontWeight:500,color:'#1e3a5f',fontSize:14}}>{product.name}</p>
                              <p style={{fontSize:12,color:'#3b82f6',fontWeight:600}}>{product.points} pts / unite</p>
                            </div>
                          </div>
                          <div style={{display:'flex',alignItems:'center',gap:8}}>
                            <button
                              onClick={() => setSelectedProducts(s => ({ ...s, [product.name]: Math.max(0, (s[product.name] || 0) - 1) }))}
                              style={{width:28,height:28,borderRadius:'50%',background:'#dbeafe',border:'none',cursor:'pointer',fontWeight:700,color:'#1e40af',fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>
                              -
                            </button>
                            <span style={{fontWeight:700,color:'#1e3a5f',minWidth:20,textAlign:'center'}}>{qty}</span>
                            <button
                              onClick={() => setSelectedProducts(s => ({ ...s, [product.name]: (s[product.name] || 0) + 1 }))}
                              style={{width:28,height:28,borderRadius:'50%',background:'#1e40af',border:'none',cursor:'pointer',fontWeight:700,color:'white',fontSize:16,display:'flex',alignItems:'center',justifyContent:'center'}}>
                              +
                            </button>
                          </div>
                        </div>
                ...
                {totalPointsFromProducts() > 0 && (
                  <div style={{background:pointsBg,border:pointsBorder,borderRadius:12,padding:14}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:4}}>
                      <span style={{fontSize:14,color:'#78716c'}}>Points a attribuer</span>
                    ...
                    <div style={{display:'flex',justifyContent:'space-between'}}>
                      ...
                      <span style={{fontWeight:700,color:'#1e3a5f',fontSize:14}}>
                        {selectedCustomer.points + totalPointsFromProducts()} pts
                      </span>
                    </div>
                  </div>
                )}
            ...
            <div style={{display:'flex',gap:12,paddingTop:4}}>
              <button onClick={() => setSelectedCustomer(null)} className="btn-secondary" style={{flex:1,justifyContent:'center'}}>
                Annuler
              </button>
              <button
