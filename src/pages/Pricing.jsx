    ...
    <div style={{minHeight:'100vh',background:'#fafaf9',display:'flex',flexDirection:'column'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'48px 24px'}}>
        <div style={{textAlign:'center',marginBottom:48}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:8,padding:'6px 16px',background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:99,color:'#ea580c',fontSize:14,fontWeight:500,marginBottom:16}}>
            🎉 15 jours gratuits · Sans carte bancaire
          </div>
          <h1 style={{fontSize:36,fontWeight:800,color:'#1c1917',marginBottom:12}}>Choisissez votre plan</h1>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:24,width:'100%',maxWidth:640}}>
          {plans.map((plan, i) => (
            <div key={i} style={{position:'relative',borderRadius:20,padding:32,border:plan.highlight?'2px solid #f97316':'2px solid #e7e5e4',background:plan.highlight?'linear-gradient(135deg,#f97316,#ea580c)':'white',boxShadow:plan.highlight?'0 20px 40px -10px rgba(249,115,22,0.3)':'0 1px 3px rgba(0,0,0,0.04)'}}>
              {plan.badge && (
              ...
              <button onClick={() => navigate('/signup')}
                style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'14px',borderRadius:12,fontWeight:700,fontSize:15,cursor:'pointer',border:'none',background:plan.highlight?'white':'#f97316',color:plan.highlight?'#ea580c':'white'}}>
                Commencer gratuitement <ArrowRight style={{width:16,height:16}} />
              </button>
              <p style={{textAlign:'center',fontSize:12,marginTop:12,color:plan.highlight?'#fed7aa':'#a8a29e'}}>
